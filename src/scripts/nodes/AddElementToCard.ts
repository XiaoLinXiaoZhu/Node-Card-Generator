...
import { addCardElement, createCardElement, drawCardOnNode, loadImageFromLink } from "./cardLibs";
...
class AddCardElementToCard extends LGraphNode {
    ...
    properties = {
        width: 200, // 卡片宽度
        height: 300, // 卡片高度
        cardConfig: {} as CardElement, // 卡片配置
    };
    ...
    addReactiveWidget(name: string, value: any): void {
        this.addWidget("text", name, value, (v) => {
            (this.properties.cardConfig as any)[name] = v; // 更新卡片配置
            if (name === "style") {
                try {
                    this.properties.cardConfig.style = JSON.parse(v); // 解析样式字符串
                } catch (e) {
                    console.error("解析样式字符串失败", e); // 解析失败
                }
            }

            //DEBUG
            console.log("更新卡片配置",name,v,this.properties.cardConfig);

            this.reAdd(); // 重新添加卡片元素
        });
        (this.properties.cardConfig as any)[name] = value; // 设置卡片配置
    }
    ...
    reAdd(): void {
        ...
        const el = createCardElement(this.properties.cardConfig,this); // 创建卡片元素
        // 更新卡片元素
        this.cardElement = el;
